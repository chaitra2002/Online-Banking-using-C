...
{
case 1:
printf("Your current balance is Rs.
...
break;
case 4:
printf("Please enter the phone number to trasnfer
...
if(fptr==NULL) printf("Sorry number is not
...
if(fwrite != 0)
{
printf("Your trasfer is completed. You have

trasnfered Rs.%d to %s",amount,usr.phone);
fclose(fptr);
user.balance -= amount;
strcpy(filename,user.phone);
fp = fopen(strcat(filename,".dat"),"w");
